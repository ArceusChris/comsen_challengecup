...
class DroneController:
    def __init__(self, aircraft_type="iris", aircraft_id=0, control_type="vel"):
        self.aircraft_type = aircraft_type  # 支持任何飞机类型
        ...
        # ROS初始化
        rospy.init_node(f'{aircraft_type}_{aircraft_id}_velocity_controller')
        ...
        rospy.loginfo(f"无人机控制器已初始化: {aircraft_type}_{aircraft_id}, 控制类型: {control_type}")
    ...
    def vel_cmd_callback(self, msg):
        ...
        if len(msg.data) >= 4:
            ...
            # 针对固定翼的特殊处理
            if "vtol" in self.aircraft_type.lower() or "plane" in self.aircraft_type.lower():
                # 固定翼需要保持最小前进速度
                forward_speed = max(abs(x), abs(y))  # 取x,y中的较大值作为前进速度
                if forward_speed < 12.0:  # 最小前进速度12m/s
                    forward_speed = 15.0  # 设置为安全巡航速度
                
                # 固定翼不侧飞，将y速度转换为偏航调整
                if abs(y) > 0.1:
                    omega += y * 0.2  # 简单的侧向到偏航转换
                    omega = max(-self.MAX_ANG_VEL, min(self.MAX_ANG_VEL, omega))
                
                self.current_twist.linear.x = forward_speed if x >= 0 else -forward_speed
                self.current_twist.linear.y = 0.0  # 固定翼不侧飞
                self.current_twist.linear.z = z
                self.current_twist.angular.z = omega
                
                rospy.loginfo(f"固定翼速度命令: 前进={self.current_twist.linear.x:.2f}, 爬升={z:.2f}, 偏航={omega:.2f}")
            else:
                # 多旋翼正常处理
                ...
                rospy.loginfo(f"多旋翼速度命令: x={x:.2f}, y={y:.2f}, z={z:.2f}, omega={omega:.2f}")
    ...
    def status_monitor(self):
        ...
        while not rospy.is_shutdown():
            status_msg = String()
            status_msg.data = f"Armed: {self.is_armed}, Mode: {self.current_mode}, " \
                            f"Vel: [{self.current_twist.linear.x:.2f}, {self.current_twist.linear.y:.2f}, " \
    ...
    def get_status_info(self):
        ...
        return f"""
========== 无人机状态 ==========
类型: {self.aircraft_type}_{self.aircraft_id}
控制模式: {self.control_type}
是否解锁: {self.is_armed}
飞行模式: {self.current_mode}
当前速度: x={self.current_twist.linear.x:.2f}, y={self.current_twist.linear.y:.2f}, 
...
"""
    ...
    def auto_takeoff_sequence(self):
        ...
        根据飞机类型调整起飞参数
        """
        rospy.loginfo(f"开始{self.aircraft_type}自动起飞流程...")
        
        # 根据飞机类型设置不同的起飞参数
        if "vtol" in self.aircraft_type.lower() or "plane" in self.aircraft_type.lower():
            # 固定翼/VTOL需要前进速度
            rospy.loginfo("步骤1: 设置固定翼起飞速度")
            self.current_twist.linear.x = 15.0  # 前进速度15m/s
            self.current_twist.linear.y = 0.0
            self.current_twist.linear.z = 2.0   # 上升速度2m/s
            self.current_twist.angular.x = 0.0
            self.current_twist.angular.y = 0.0
            self.current_twist.angular.z = 0.0
            rospy.loginfo(f"固定翼起飞参数: 前进={self.current_twist.linear.x}m/s, 上升={self.current_twist.linear.z}m/s")
        else:
            # 多旋翼起飞
            rospy.loginfo("步骤1: 设置多旋翼上升速度到0.5m/s")
        ...
        rospy.loginfo(f"{self.aircraft_type}自动起飞流程完成！")
    ...
    def auto_landing_sequence(self):
        ...
        self.current_twist.angular.z = 0.0
        rospy.sleep(5.0)  # 下降5秒
        
        # 步骤3: 降落模式
        rospy.loginfo("步骤3: 切换到降落模式")
        self.land()
        rospy.sleep(3.0)
        
        # 步骤4: 上锁
        rospy.loginfo("步骤4: 上锁无人机")
        self.disarm()
...
def main():
    """主函数"""
    if len(sys.argv) < 4:
        print("用法: python3 drone_control.py <aircraft_type> <aircraft_id> <control_type> [auto_takeoff]")
        print("例如: python3 drone_control.py iris 0 vel")
        print("     python3 drone_control.py standard_vtol 0 vel auto_takeoff  # 自动起飞")
        print("飞机类型: iris, standard_vtol, plane, typhoon_h480 等")
    ...
    # 创建控制器
    controller = DroneController(aircraft_type, aircraft_id, control_type)
